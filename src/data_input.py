...
class DataSet:
    ...
    def create_dataset(self,
        ...
        if len(self.filenames.shape) > 1:
            print('Three-resolution data input.')
            ...
            dataset = dataset.map(lambda x1, x2, x3, z, y, w:
                              self.multi_parse_function(filename1=x1, filename2=x2, filename3=x3,
                                                  cov=z, label=y, weight=w, augmentation=augmentation))   #num_parallel_calls=8
            
        else:
            print('Single resolution data input.')
            ...
            if shuffle:
                dataset = dataset.shuffle(len(self.labels),
                                      reshuffle_each_iteration=True)  # perfect shuffle
            dataset = dataset.repeat(ds_epoch)
            dataset = dataset.map(lambda x, z, y, w:
                              self.single_parse_function(filename=x,
                                                  cov=z, label=y, weight=w, augmentation=augmentation))   #num_parallel_calls=8
    ...
    def single_parse_function(self,
                       filename, cov, label, weight, augmentation=False):
        ...
        if not self.legacy:
            image = clip_and_resize(image)
            
            if self.sym:
                image = symmetric(image)
